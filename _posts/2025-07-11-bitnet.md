...
There are many open-source models available, including the excellent DeepSeek. However, it is unrealistic to run something like DeepSeek V3 on your own hardware. You could run a reduced version or distil it, but either option would result in a significant performance trade-off or require industrial GPUs.

Since the trend seems to be embedding AI everywhere, this is a problem. [I don't think LLMs are the best architecture for developing everything](https://dariotortorici.github.io/blog/perceptron-is-dead/), but they are the most promising at the moment, so we will work with them.
